...
function changemoney() {
  let AllMoney = 0;
  for (let i = 0; i < Itemlist.length; i++) {
    let currentItem = document.getElementById(Itemlist[i].item);
    if (currentItem) {
      AllMoney += ShopData.stock[Itemlist[i].item.toLowerCase()].price * parseFloat(currentItem.innerHTML);
    }
  }
  
  // Assuming MoneyForm is defined somewhere
  $('.buy_section .shop_container h3').html(AllMoney.toLocaleString() + '<money>' + MoneyForm + '</money>');
}
...
function BuyInShop() {
  ...
  for (let i = 0; i < Itemlist.length; i++) {
    let currentItem = document.getElementById(Itemlist[i].item);
    if (currentItem && currentItem.innerHTML > 0) {
      BuyItems.push([Itemlist[i].item, currentItem.innerHTML, ShopData.stock[Itemlist[i].item.toLowerCase()].price]);
    }
  }
...
function SaveStockPrice(id, num){
  if (document.getElementById(id).value > 0){
    ManagementTable.stock[num].price = document.getElementById(id).value
...
function CreateOrder(){
  ...
  for (var k in ManagementTable.stock) {
    if (document.getElementById("order_num_"+k).value > 0){
      price = price + +ManagementTable.stock[k].price * +document.getElementById("order_num_"+k).value
  ...
  if (ChoosedOrderType == "me"){
    $.post('https://'+GetParentResourceName()+'/UseButton', JSON.stringify({action:"CreateOrder", type:ChoosedOrderType, table, price}))
  }
  else{
